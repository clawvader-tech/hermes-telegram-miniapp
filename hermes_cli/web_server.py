...
@app.get("/api/status")
async def get_status(_: None = Depends(_require_auth_local_ok)):
...
async def get_sessions(limit: int = 20, offset: int = 0, _: None = Depends(_require_auth_local_ok)):
    try:
        from hermes_state import SessionDB
        db = SessionDB()
        try:
...
async def search_sessions(q: str = "", limit: int = 20, _: None = Depends(_require_auth_local_ok)):
...
@app.get("/api/config")
async def get_config(_: None = Depends(_require_auth_local_ok)):
...
async def get_defaults(_: None = Depends(_require_auth_local_ok)):
    return DEFAULT_CONFIG


@app.get("/api/config/schema")
async def get_schema(_: None = Depends(_require_auth_local_ok)):
...
@app.get("/api/env")
async def get_env_vars(_: None = Depends(_require_auth_local_ok)):
...
async def list_oauth_providers(_: None = Depends(_require_auth_local_ok)):
...
async def poll_oauth_session(provider_id: str, session_id: str, _: None = Depends(_require_auth_local_ok)):
...
async def get_session_detail(session_id: str, _: None = Depends(_require_auth_local_ok)):
    from hermes_state import SessionDB
    db = SessionDB()
    try:
...
async def get_session_messages(session_id: str, _: None = Depends(_require_auth_local_ok)):
    from hermes_state import SessionDB
    db = SessionDB()
    try:
        sid = db.resolve_session_id(session_id)
...
async def get_logs(
    ...
    _: None = Depends(_require_auth_local_ok),
):
...
async def list_cron_jobs(_: None = Depends(_require_auth_local_ok)):
    from cron.jobs import list_jobs
    return list_jobs(include_disabled=True)


@app.get("/api/cron/jobs/{job_id}")
async def get_cron_job(job_id: str, _: None = Depends(_require_auth_local_ok)):
...
@app.get("/api/skills")
async def get_skills(_: None = Depends(_require_auth_local_ok)):
...
async def get_toolsets(_: None = Depends(_require_auth_local_ok)):
...
async def get_config_raw(_: None = Depends(_require_auth_local_ok)):
...
async def get_usage_analytics(days: int = 30, _: None = Depends(_require_auth_local_ok)):
    from hermes_state import SessionDB
    db = SessionDB()
    try:
...
async def get_session_usage(_: None = Depends(_require_auth_local_ok)):
...
async def get_system_health(_: None = Depends(_require_auth_local_ok)):
...
async def get_model_info(_: None = Depends(_require_auth_local_ok)):
...
@app.get("/api/agents")
async def list_agents(_: None = Depends(_require_auth_local_ok)):
...
async def get_agent(name: str, _: None = Depends(_require_auth_local_ok)):
...
async def list_processes(_: None = Depends(_require_auth_local_ok)):
